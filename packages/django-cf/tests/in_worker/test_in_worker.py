...
def test_django_wsgi_reads_post_request_body(dev_server: str) -> None:
    response = requests.post(
        f"{dev_server}/django/body/",
...
def test_django_wsgi_preserves_binary_response(dev_server: str) -> None:
    response = requests.get(f"{dev_server}/django/binary/", timeout=10)
...
def test_django_wsgi_streams_response(dev_server: str) -> None:
    response = requests.get(f"{dev_server}/django/stream/", stream=True, timeout=10)
...
def test_django_wsgi_preserves_multiple_cookies(dev_server: str) -> None:
    response = requests.get(f"{dev_server}/django/cookies/", timeout=10)
...
def test_django_wsgi_builds_request_metadata(dev_server: str) -> None:
    response = requests.get(
        f"{dev_server}/django/meta/%E6%9D%B1%E4%BA%AC/",
...
def test_django_wsgi_reads_delete_request_body(dev_server: str) -> None:
