...
class CFSQLCompiler(SQLCompiler):
    ...
    def _compile_date_trunc(self, func, **extra_context):
        ...
        templates = {
            ...
            "quarter": 'CASE ((CAST(STRFTIME("%%m", {}) AS INTEGER) - 1) / 3 + 1) WHEN 1 THEN STRFTIME("%%Y-01-01", {}) WHEN 2 THEN STRFTIME("%%Y-04-01", {}) WHEN 3 THEN STRFTIME("%%Y-07-01", {}) WHEN 4 THEN STRFTIME("%%Y-10-01", {}) END'.format(
                field_sql, field_sql, field_sql, field_sql, field_sql
            ),
            "month": 'STRFTIME("%Y-%m-01", {})'.format(field_sql),
            "week": 'DATE({}, "-" || CAST((CAST(STRFTIME("%w", {}) AS INTEGER) + 6) %% 7 AS TEXT) || " days")'.format(
                field_sql, field_sql
            ),
