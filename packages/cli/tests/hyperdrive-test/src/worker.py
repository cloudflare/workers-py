"""Hyperdrive test worker.

Each database driver suite lives in a `test_<suite>.py` module written as ordinary pytest
tests (see test_postgresql.py, test_mysql.py). The `/run-tests/<suite>` endpoint runs pytest
against that module inside workerd and returns per-test results as JSON, which the host-side
test_hyperdrive.py maps onto individual pytest cases.

To add a new driver: create `src/test_<driver>.py` with pytest tests.
"""
...
class ResultCollector:
    ...
    tests/test_hyperdrive.py (e.g. test_basic_query -> "basic_query").
    """
