...
@pytest.mark.asyncio
async def test_api_view_and_function_view(django_app):
    cases = [
    ...
    for method, path, data, status, expected in cases:
        if method == "GET":
            response, payload = await get_json(django_app, path)
        elif method == "DELETE":
            response = await fetch(django_app, path, method=method)
            payload = None
        elif method == "PUT":
            ...
            response = await post_json(django_app, path, data)
            payload = json.loads(await response.text())
...
@pytest.mark.asyncio
async def test_method_not_allowed(django_app):
    response = await fetch(django_app, "/drf/function-view/", method="PATCH")
