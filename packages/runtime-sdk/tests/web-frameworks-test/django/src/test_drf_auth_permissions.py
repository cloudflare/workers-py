...
@pytest.mark.asyncio
async def test_auth_and_permission_cases(django_app):
    cases = [
    ...
    for path, headers, status, expected in cases:
        if expected is None:
            response = await fetch(django_app, path, headers=headers)
            if isinstance(status, set):
                assert response.status in status
            else:
                assert response.status == status
        else:
            response, payload = await get_json(django_app, path, headers=headers)
            assert response.status == status
