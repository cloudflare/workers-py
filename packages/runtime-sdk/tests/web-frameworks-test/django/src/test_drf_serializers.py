...
@pytest.mark.asyncio
async def test_serializer_and_nested_cases(django_app):
    serializer_cases = [
        (
    ...
    for data, valid, check in serializer_cases:
        response = await post_json(django_app, "/drf/serializer/", data)
        payload = json.loads(await response.text())
        assert payload["valid"] is valid
        assert check(payload)

    response, payload = await get_json(django_app, "/drf/serializer/")
    ...
    for data, valid, check in nested_cases:
        response = await post_json(django_app, "/drf/nested/", data)
        payload = json.loads(await response.text())
        assert payload["valid"] is valid
        assert check(payload)
