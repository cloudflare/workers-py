...
@pytest.mark.asyncio
async def test_form_validation(django_app):
    for label, body in (
        (
            "valid",
        ...
        response = await post_form(django_app, "/form/validate/", body)
...
@pytest.mark.asyncio
async def test_form_get_renders_html(django_app):
    response = await fetch(django_app, "/form/validate/")
