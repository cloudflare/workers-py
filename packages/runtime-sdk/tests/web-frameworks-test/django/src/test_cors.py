...
@pytest.mark.asyncio
async def test_cors_allowed_origins(django_app):
    cases = (
    ...
    for method, origin, with_credentials in cases:
        ...
        response = await fetch(django_app, "/hello/", method=method, headers=headers)
...
@pytest.mark.asyncio
async def test_cors_disallowed_origin(django_app):
...
@pytest.mark.asyncio
async def test_cors_no_origin_no_headers(django_app):
