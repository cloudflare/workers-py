...
@pytest.mark.asyncio
async def test_session_set_and_get_values(django_app):
    for data in ({"color": "blue"}, {"color": "blue", "size": "large"}):
        set_response = await post_json(django_app, "/session/set/", data)
...
@pytest.mark.asyncio
async def test_session_flush(django_app):
    set_response = await post_json(django_app, "/session/set/", {"color": "blue"})
...
@pytest.mark.asyncio
async def test_session_overwrites(django_app):
    first_response = await post_json(django_app, "/session/set/", {"color": "blue"})
...
@pytest.mark.asyncio
async def test_session_cookie_present(django_app):
    response = await post_json(django_app, "/session/set/", {"color": "blue"})
...
@pytest.mark.asyncio
async def test_session_empty_initially(django_app):
    response, payload = await get_json(django_app, "/session/get/")
