...
@pytest.mark.asyncio
async def test_content_negotiation_cases(django_app):
    json_data = {"name": "widget", "quantity": 2}
    response = await post_json(django_app, "/drf/content/", json_data)
    ...
    response = await post_json(django_app, "/drf/content/", {"ping": "pong"})
