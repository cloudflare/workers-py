...
def test_sync_recreates_venv_on_python_version_mismatch(clean_test_dir):
    ...
    """
    # Setup: Create initial files in the clean test directory
    create_test_pyproject([])
    create_test_wrangler_jsonc()
    ...
    try:
        ...
        assert result1.returncode == 0, f"First sync failed: {result1.stdout}\n{result1.stderr}"
        ...
        assert result2.returncode == 0, f"Second sync failed: {result2.stdout}\n{result2.stderr}"
        ...
        # Optional: Verify the python version in the new venv
        ...
        assert "3.13" in version_result.stdout, f"Python version is not 3.13: {version_result.stdout}"
